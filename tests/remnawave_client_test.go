...
func TestCreateUserDescription(t *testing.T) {
	api := &stubAPI{}
	c := remnawave.NewClientWithAPI(api)
	ctx := context.WithValue(context.Background(), contextkey.Username, "user")
	if _, err := c.CreateUserInternal(ctx, 1, 1, 1); err != nil {
	...
	api.createReq = nil
	if _, err := c.CreateUserInternal(context.Background(), 1, 1, 1); err != nil {
		t.Fatalf("createUser: %v", err)
	}
	if api.createReq.Description.IsSet() {
...
func TestUpdateUserDescription(t *testing.T) {
	...
	if _, err := c.UpdateUserInternal(ctx, existing, 1, 1); err != nil {
